...
spr = 100;
a = arduino('COM13', 'Uno');
imu = mpu9250(a, 'SampleRate', fs, 'OutputFormat', 'matrix', 'ReadMode', 'latest', 'SamplesPerRead', spr);
...
for n = 1:N*5
    ...
    if(mod(n, 5)==0)
        ...
        [accel_list, gyro_list, mag_list] = read(imu);
        for j=1:5:spr/5
            accel = accel_list((j-1)*5+1:j*5, :);
            gyro = gyro_list((j-1)*5+1:j*5, :);
            mag = mag_list((j-1)*5+1:j*5, :);
...
%% Data Processing
x = reshape([free_gyro, free_accel], N, 6, spr/5);
figure
for i=1:N
    subplot(10, 2, i)
    s = normalize(reshape(x(i,:,:), 6, spr/5), 'norm', 2);
    imshow(s)
end
